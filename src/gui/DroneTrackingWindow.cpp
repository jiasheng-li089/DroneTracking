...
DroneTrackingWindow::DroneTrackingWindow(std::string config_file,
  ...
  connect(m_rs_manager.get(), &RealSenseManager::frames_received, this,
          &DroneTrackingWindow::frames_received);
  connect(m_rs_manager.get(), &RealSenseManager::error_occurred, this,
  ...
  connect(m_vision_tracker.get(), &tracking::VisionTracker::frames_received,
          this, &DroneTrackingWindow::frames_received);
...
void DroneTrackingWindow::on_button_clicked(QWidget *sender) {
  ...
  case 0:
    ...
    m_rs_manager->set_frame_callback(std::bind(
...
void DroneTrackingWindow::frames_received(
    std::vector<std::tuple<int, std::string, QImage>> frames) {
  if (m_start_camera_btn->isEnabled() || frames.size() == 0)
    return;
