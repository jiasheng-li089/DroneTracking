...
std::string get_special_folder(special_folder f) {
  ...
  if (f == special_folder::temp_folder) {
    ...
    auto len = GetTempPathA(sizeof(buf),
                            buf); // "return the length of the string, not
    ...
    // The user's Documents folder location may get overridden, as we know
    // OneDrive does in certain circumstances. In such cases, the new function,
    // SHGetKnownFolderPath, does not always return the new path, while the
    ...
    case special_folder::app_data:
      ...
      break;
    default:
      throw std::invalid_argument("special_folder value (" +
                                  std::to_string((int)f) +
                                  ") is not supported");
    }
    ...
    PWSTR path = NULL;
    HRESULT hr =
        SHGetKnownFolderPath(folder, KF_FLAG_DEFAULT_PATH, NULL, &path);
    ...
    CoTaskMemFree(path);
    if (error)
      throw std::runtime_error("failed to convert special folder: errno=" +
                               std::to_string(error));
    res = str;
    res += "\\";
  }
