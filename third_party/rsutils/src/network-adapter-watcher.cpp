...
class network_adapter_watcher_singleton {
  using public_signal =
      rsutils::public_signal<network_adapter_watcher_singleton>;
...
public:
  ...
  network_adapter_watcher_singleton()
#ifdef _WIN32
      : _done(CreateEvent(nullptr /*security attrs*/, TRUE /*manual reset*/,
                          FALSE /*init unset*/, nullptr /*no name*/)),
        _th([this] {
          ...
          overlap.hEvent =
              CreateEvent(nullptr /*security attrs*/, FALSE /*auto reset*/,
                          FALSE /*init unset*/, nullptr /*no name*/);
          ...
          while (true) {
            ...
            // If multiple objects become signaled, the function returns the
            // index of the first handle in the array whose object was signaled:
            auto result = WaitForMultipleObjects(
                n_handles, handles, FALSE /*wait-for-all*/, INFINITE);
            ...
            if ((WAIT_OBJECT_0 + 1) == result) {
              ...
              LOG_ERROR("Network adapter wait failed ("
                        << result << "); GetLastError is " << last_error);
              break;
            }
          }
      ...
      : _socket(open_netlink()), _done(eventfd(0, 0))
#endif
  {
#ifndef _WIN32
    if (_socket < 0) {
  ...
  ~network_adapter_watcher_singleton() {
    ...
    if (_th.joinable()) {
      ...
      if (rv != sizeof(incr))
        LOG_WARNING(
            "failed to write to network adapter watcher done event: " << rv);
      else
        _th.join();
    }
    close(_socket);
...
static rsutils::shared_ptr_singleton<detail::network_adapter_watcher_singleton>
    the_adapter_watcher;
...
#endif
{
  // As long as someone keeps a pointer to an network_adapter_watcher, the
  // singleton will be kept alive and it will watch for changes; as soon as all
  // instances disappear, the singleton will disappear and the watch should
  // stop.
}
